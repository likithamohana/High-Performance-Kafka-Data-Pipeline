...
# Broker must accept metadata + produce; port open is not enough right after start.
sleep 15
...
time /app/kafka-sort -mode=full -broker=localhost:9092 -chunk=1000000 -source-partitions=4 -concurrency=2 -total="${TOTAL_RECORDS}" 2>&1 | tee /app/data/pipeline.log
